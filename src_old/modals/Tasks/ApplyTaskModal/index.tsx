...
import { applyTaskAction } from "store/actions/task";
...
export default ({ open, hideBackdrop, closeModal }: Props) => {
    ...
    // @ts-ignore
    const { Task, applyTaskLoading } = useAppSelector(store => store.task);
    ...
    const handleApplyApplication = () => {
        ...
        // }
            ...
            dispatch(applyTaskAction({ taskId: _get(Task, '_id', ''), daoUrl: _get(DAO, 'url', ''), payload: { note, resourceList } }))
        }
    }

    const applyAsync = _debounce(handleApplyApplication, 1500)
        ...
        >
            <Box className={classes.modalConatiner}>
                ...
                <Box sx={{ marginTop: '70px' }} display="flex" flexDirection="column" alignItems="center">
                    ...
                    <Typography className={classes.modalTitle}>Apply to Task</Typography>
                </Box>
                ...
                <Box display="flex" flexDirection="column" sx={{ width: '80%' }}>
                    ...
                    <Box display="flex" flexDirection="row" alignItems="center" justifyContent={"space-between"} sx={{ height: '80px' }}>
                        <TextInput
                            sx={{ width: 145 }}
