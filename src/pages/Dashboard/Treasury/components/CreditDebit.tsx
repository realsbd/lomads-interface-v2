    ...
    return (
        <TableCell>
            <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                ...
                <Typography style={{ fontFamily: 'Inter,sans-serif', fontWeight: 700, fontSize: 14, color: '#76808d',  }} sx={{ width: 50 }} ml={1}>{ isNaN(amount) || !token ? '-' : `${parseFloat(amount).toFixed(3)} ${token}` }</Typography>
            </Box>
        </TableCell>
    )
}