...
export default ({ open, closeModal }: Props) => {
    ...
    const handleCloseTask = () => {
        dispatch(archiveTaskAction({ taskId: _get(Task, '_id', ''), daoUrl: _get(DAO, 'url', '') }));
    }
    ...
    return (
        ...
        >
            <Box sx={style} display={"flex"} flexDirection={"column"} alignItems={"center"}>
                ...
                <Typography className={classes.modalTitle}>Close {_get(Task, 'name', '')}</Typography>
                <Typography className={classes.modalSubtitle}>This action is irreversible for now.</Typography>
                <Typography className={classes.modalSubtitle}>You will find closed tasks in the archives.</Typography>
                <Box display={"flex"} alignItems={"center"} justifyContent={"center"} style={{ width: '100%', marginTop: '20px' }}>
                    ...
                    <Button variant="contained" onClick={handleCloseTask} loading={archiveTaskLoading}>YES</Button>
                </Box>
            </Box>
        </Modal>
    )
