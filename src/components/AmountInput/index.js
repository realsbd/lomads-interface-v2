...
export default ({ onChange, value, height = 40 }) => {
    const classes = useStyles()
    return (
        <NumberInput value={value} onChange={e => onChange(e)} style={{ width: (54 + 50), height, borderRadius: '10px 10px 10px 10px', boxShadow: 'inset -1px 0px 4px rgba(27, 43, 65, 0.1)' }} step={1} min={0}>
            <NumberInputField className='input' style={{ padding: 0, textAlign: "center", height, width: 54, backgroundColor: '#F5F5F5', borderRadius: '10px 0px 0px 10px', borderWidth: 0 }} />
